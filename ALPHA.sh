...
output_file="container_status.html"
generate_html_page "$CONTAINER_INFO" "$output_file"

# Copy the HTML page to Cockpit's static directory
cp "$output_file" "/usr/share/cockpit/static/"

# Add a cronjob to periodically update the HTML page
cronjob_entry="*/5 * * * * root curl -sS -k http://localhost/container_status.html -o /usr/share/cockpit/static/container_status.html"
(crontab -l 2>/dev/null; echo "$cronjob_entry") | crontab -

echo "HTML page generated: $output_file"
echo "Cronjob added to update the HTML page."
